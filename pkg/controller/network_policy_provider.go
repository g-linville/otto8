...
func (h *helmNetworkPolicyProviderInstaller) InstallOrUpgrade(ctx context.Context, spec networkPolicyProviderInstallSpec) error {
	actionConfig, err := h.newActionConfig(spec.ReleaseNamespace)
	if err != nil {
		return err
	}
	...
	if err != nil && !errors.Is(err, driver.ErrReleaseNotFound) {
		return fmt.Errorf("failed to inspect network policy provider release %q: %w", spec.ReleaseName, err)
	}

	if rel != nil {
		rel, err = recoverPendingRelease(actionConfig, rel)
		if err != nil {
			return fmt.Errorf("failed to recover pending network policy provider release %q: %w", spec.ReleaseName, err)
		}
	}
	...
	if rel == nil {
		...
		if _, err := install.RunWithContext(ctx, ch, spec.Values); err != nil {
			return fmt.Errorf("failed to install network policy provider: %w", err)
		}
		return nil
	}
	...
	if _, err := upgrade.RunWithContext(ctx, spec.ReleaseName, ch, spec.Values); err != nil {
		return fmt.Errorf("failed to upgrade network policy provider: %w", err)
	}
...
func (h *helmNetworkPolicyProviderInstaller) Uninstall(_ context.Context, releaseNamespace string) error {
	actionConfig, err := h.newActionConfig(releaseNamespace)
	if err != nil {
		return err
	}
	...
	if _, err := uninstall.Run(networkPolicyProviderReleaseName); err != nil {
		...
		return fmt.Errorf("failed to uninstall network policy provider release %q: %w", networkPolicyProviderReleaseName, err)
	}
...
func (h *helmNetworkPolicyProviderInstaller) newActionConfig(namespace string) (*action.Configuration, error) {
	...
	if err := actionConfig.Init(flags, namespace, "secret", log.Debugf); err != nil {
